    ...
    try:
        ...
        if prompt == '0':
            if len(game_state.story) > 0:
                conclusion_prompt = get_story_prompt(game_state.story, game_state.score, is_conclusion=True)
        ...
        # Find card in deck
        card = next((c for c in CARD_DECK if int(c['numero']) == card_number), None)
        if not card:
        ...
        # Generate story text
        story_prompt = get_story_prompt(game_state.story, game_state.score, card, player_role, effect)
        ...
        game_state.log_action(f"{player_name} ({player_role}) a joué la carte {card_number} - {card['mot']}")
...
def refresh():
    ...
    try:
        ...
        if game_state.should_auto_reset():
            ...
            game_state.log_action("Jeu réinitialisé automatiquement après inactivité")
...
def cards():
    """Get available cards"""
    try:
        available_cards = [card for card in CARD_DECK if int(card['numero']) not in game_state.played_cards]
        return jsonify(available_cards)
    except Exception as e:
        logger.error(f"Error getting cards: {e}")
        return jsonify({'error': 'Erreur lors de la récupération des cartes'}), 500
